...
def generate(
    ...
    """
    Generates text token-by-token and prints each piece live
    as it's produced (like ChatGPT-style streaming).
    """
    ...
    prev_text = ""  # tracks decoded text so far, to print only new chars
    ...
    for _ in range(max_new_tokens):
        ...
        assistant_text = full_text.split("### Assistant:")[-1]

        new_piece = assistant_text[len(prev_text):]
        print(new_piece, end="", flush=True)
        prev_text = assistant_text

    print()  # final newline after generation finishes

    # The return part is commented to avoid 2 responses in notebooks
